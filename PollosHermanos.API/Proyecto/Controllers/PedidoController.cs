...
namespace API.Controllers
{
    ...
    public class PedidoController : ControllerBase, IPedidoController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Agregar([FromBody] PedidoRequest pedido)
        {
        ...
        [HttpPut("{Id}")]
        public async Task<IActionResult> Editar([FromRoute] Guid Id, [FromBody] PedidoRequest pedido)
        {
        ...
        public async Task<IActionResult> Eliminar([FromRoute] Guid Id)
        {
            if (!await VerificarExistencia(Id))
                return NotFound("Pedido no existe");
            var resultado = await _pedidoFlujo.Eliminar(Id);
            return NoContent();
        }
        ...
        private async Task<bool> VerificarExistencia(Guid Id)
        {
            ...
            var Existe = await _pedidoFlujo.Obtener(Id);
            if (Existe != null)
