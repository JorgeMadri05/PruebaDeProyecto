...
namespace BC
{
    public class AutenticacionReglas : IAutenticacionBC
    {
        ...
        public async Task<Token> LoginAync(LoginBase login)
        {
            ...
            _usuario= await _usuarioDA.ObtenerUsuario(new Cliente { Nombre = login.NombreUsuario, Correo = login.CorreoElectronico });
            if (_usuario == null)
        ...
        {
            return (login != null && login.PasswordHash == _usuario.Password);
        ...
        private async Task<List<Claim>> GenerarClaims()
        {
            ...
            claims.Add(new Claim(ClaimTypes.Email, _usuario.Correo));            
            return claims;
        }
